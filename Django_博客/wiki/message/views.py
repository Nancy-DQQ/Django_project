...
def messages(request,topic_id):
    ...
    if request.method == 'POST':
        ...
        # 第二种方案 用键值 赋值给 外键字段名
        # Message.objects.create(content = content,parent_message = parent_id,publisher_id = request.user.username,
        #                        topic_id = topic_id)
    ...
    if request.method == 'GET':
        ...
        # v1/message/topic_id
        all_m = Message.objects.filter(id = topic_id)
        all_list = []
        for m in all_m:
            d = {}
            d['id'] = m.id
