...
def topics(request, author_id):
    if request.method == 'POST':
        ...
        # 创建topic
        Topic.objects.create(title=title, limit=limit, content=content, category=category, introduce=introduce,
                             author=author)
    ...
    if request.method == 'GET':
        # 获取 用户文章数据
        # /v1/topics/guoxiaonao - guoxiaonao 的所有文章
        ...
        if not authors:
            result = {'code': 30104, 'error': 'The author us not existed!'}
            return JsonResponse(result)
        # 当前被访问的博主
        author = authors[0]
        ...
        if t_id:
            t_id = int(t_id)
            # 获取制定文章的详情页
            # 生成标记为True 为博主访问自己,Flase为陌生访客访问博主
            is_self = False
    ...
    for message in all_messages:
        if message.parent_message:
            ...
            reply_home[message.parent_message].append({
                                ...
                                'created_time':message.created_time.strftime(
                                '%Y-%m-%d %H:%M:%S'
                                                        )})
        else:
            #  留言
